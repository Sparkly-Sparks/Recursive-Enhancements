...
def blend(Card,Layer,Type="Other"):
...
for i in range(256):
    ...
    Glass=False
    if trim_stone:
        Card=trim(Card,"Stone",Glass)
        trim_stone=False
    if trim_lucky:
        if current[4]=="1":
            Glass=True
        Card=trim(Card,"Lucky",Glass)
        trim_lucky=False
    Canvas.paste(Card,coords(i),Card)
Canvas.save("Elements/temp/Enhancements.png")
